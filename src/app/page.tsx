...
const ROOM_ID = "strongers-3";
const ROOM_NAME = "Strongers 3";
...
export default function Home() {
  ...
  // Auto-generate room code on mount if not specified
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`quebra-gelo:${ROOM_ID}`, {
      config: {
  ...
  }, [isJoined, userName, userColor, isMaster]);
          ...
          <div className="w-full max-w-md bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 shadow-2xl relative">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-slate-400 mt-2">
                Desenhe em tempo real com sua célula!
              </p>
            </div>
          ...
          <header className="h-16 border-b border-slate-800 bg-slate-900/80 backdrop-blur px-4 flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="h-9 px-3 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition-colors"
              >
                Sair
              </button>
            </div>
